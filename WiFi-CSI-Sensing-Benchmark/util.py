...
def load_data_n_model(dataset_name, model_name, root):
    ...
    if dataset_name == 'UT_HAR_data':
        ...
        # total trainset size
        print(" train data set length of UT_HAR_data",len(train_set))
        ...
        # Assuming you already have the train_loader defined as in your code.

        # Calculate the size of your validation set (15% of the total dataset)
        train_dataset = train_loader.dataset
        dataset_size = len(train_dataset)
        validation_size = int(0.15 * dataset_size)

        # Create the validation and training datasets using random_split
        validation_dataset, training_dataset = random_split(
            train_dataset, [validation_size, dataset_size - validation_size]
        )

        # Create DataLoader instances for the validation and training datasets
        batch_size = 64  # Adjust the batch size as needed
        validation_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True)
        train_loader = DataLoader(training_dataset, batch_size=batch_size, shuffle=True)

        # Print the lengths of the datasets
        print("Training data set length of UT_HAR_data:", len(train_loader.dataset))
        print("Validation data set length of UT_HAR_data:", len(validation_loader.dataset))


        test_loader = torch.utils.data.DataLoader(test_set,batch_size=256,shuffle=False)
        print(" test data set length of UT_HAR_data:",len(test_loader.dataset))
        if model_name == 'MLP':
        ...
        # Print the lengths of the datasets
        print("Training data set length of NTU-Fi-HumanID:", len(train_loader.dataset))
        print("Validation data set length of NTU-Fi-HumanID:", len(validation_loader.dataset))

        test_loader = torch.utils.data.DataLoader(dataset=CSI_Dataset(root + 'NTU-Fi-HumanID/train_amp/'), batch_size=64, shuffle=False)
        # total testset size
        print(" train data set length of NTU-Fi-HumanID:",len(test_loader.dataset))
        if model_name == 'MLP':
            print("using model: MLP")
        ...
        # Print the lengths of the datasets
        print("Training data set length of NTU-Fi_HAR:", len(train_loader.dataset))
        print("Validation data set length of NTU-Fi_HAR:", len(validation_loader.dataset))
        test_loader = torch.utils.data.DataLoader(dataset=CSI_Dataset(root + 'NTU-Fi_HAR/test_amp/'), batch_size=64, shuffle=False)
        # total testset size
        print(" train data set length of NTU-Fi_HAR:",len(test_loader.dataset))
        if model_name == 'MLP':
            print("using model: MLP")
            model = NTU_Fi_MLP(num_classes)
            train_epoch = 30 #10
        ...
        full_train_dataset = Widar_Dataset(root + 'Widardata/train/')
        
        # Calculate the number of samples for the validation set (15% of the training data)
        num_train_samples = len(full_train_dataset)
        print("total train samples for Widar ",num_train_samples)
        num_val_samples = int(0.15 * num_train_samples)
        print(num_val_samples)
        
        # Split the dataset into training and validation sets
        train_dataset, val_dataset = torch.utils.data.random_split(full_train_dataset, [num_train_samples - num_val_samples, num_val_samples])
        
        # Create data loaders for training, validation, and test
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=64, shuffle=True)
        # total trainset size
        # print(" train data set length of Widar ",len(train_loader.dataset))
        ...
        # total testset size
        print(" train data set length of Widar",len(test_loader.dataset))
        #  test loader size 
        print(" test data set length of Widar ",len(test_loader.dataset))
        if model_name == 'MLP':
            print("using model: MLP")
